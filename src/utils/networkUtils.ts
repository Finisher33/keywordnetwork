...
export function calculateUserNetworkData(currentUser: User, db: any) {
  ...
  const groupedNetwork = myRepKeywords.map((repKw: string) => {
    ...
    if (usersWithKw.length > 0) {
      ...
      const uniqueGivers = new Set(usersWithKw.filter(u => u.type === 'giver').map(u => u.user.id)).size;
      const uniqueTakers = new Set(usersWithKw.filter(u => u.type === 'taker').map(u => u.user.id)).size;
